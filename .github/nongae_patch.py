...
# Decode the two user-selected Nongae character assets.
b1=Path('.github/nongae-01.webp.b64').read_text().strip()
b2=''.join(Path(f'.github/nongae-02.part{i}').read_text().strip() for i in (1,2,3))
a1=base64.b64decode(b1,validate=True)
a2=base64.b64decode(b2,validate=True)
...
skins.extend([
    {'id':'nongae_jade','name':'옥가락지 논개','price':3200,'src':'assets/nongae-01.webp?v=20260904-nongae1','group':'nongae','badge':'🌊 논개','note':'RPG 논개 클리어 후 해금'},
    {'id':'nongae_resolve','name':'결의의 논개','price':3800,'src':'assets/nongae-02.webp?v=20260904-nongae1','group':'nongae','badge':'🌊 논개','note':'RPG 논개 클리어 후 해금'}
])
newarr=json.dumps(skins,ensure_ascii=False,separators=(',',':'))
s=s[:m.start(1)]+newarr+s[m.end(1):]

# Persistent unlock flag + migration for users who already cleared Nongae.
...
# Unlock message only after the existing true ending has fully finished.
...
# Spacefighter: Nongae character normal shots are hollow jade rings.
...
# Preserve the tall Nongae proportions in the vertical shooter.
...
# Remove temporary base64 staging files from final tree.
for q in ['.github/nongae-01.webp.b64','.github/nongae-02.webp.b64','.github/nongae-02.part1','.github/nongae-02.part2','.github/nongae-02.part3']:
