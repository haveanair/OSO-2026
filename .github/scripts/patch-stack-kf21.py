...
def between_replace(s, start, end, replacement, label):
    a = s.find(start)
    if a < 0:
...
# ------------------------------------------------------------------
# 1) 비단쌓기 판정 출력 통일
# GREAT/PERFECT 전용 레이어와 GOOD/MISS 기본 combo 레이어가 분리되어
# 빠른 연속 판정에서 이전 효과가 다음 판정을 가리는 문제를 제거한다.
# ------------------------------------------------------------------
...
start = "  function showStackGreatBurst(message){"
end = "  function rewriteGrowMessage(msg,extra){"
replacement = r'''  function showStackJudgementBurst(message){
    try{
      const scene=document.querySelector('#stage #sseScene');if(!scene)return false;
      const msg=String(message||'').trim();
      let kind='',main='',sub='';
      if(/^PERFECT/.test(msg)){
        kind='perfect';const parts=msg.split(' · ').map(v=>v.trim()).filter(Boolean);main=parts.shift()||'PERFECT!';sub=parts.join(' · ')||'정확하게 맞췄습니다!'
      }else if(/^GREAT!/.test(msg)){
        kind='great';main='GREAT!';sub=msg.replace(/^GREAT!\s*/,'').trim()||'좋은 위치입니다!'
      }else if(/^GOOD!/.test(msg)){
        kind='good';main='GOOD!';sub=msg.replace(/^GOOD!\s*/,'').trim()||'좋습니다!'
      }else if(/^아슬아슬!/.test(msg)){
        kind='close';main='아슬아슬!';sub=msg.replace(/^아슬아슬!\s*/,'').trim()||'조금만 더 중앙으로!'
      }else if(/^MISS!/.test(msg)){
        kind='miss';main='MISS!';sub=msg.replace(/^MISS!\s*/,'').trim()||'다시 도전!'
      }else return false;
...
'''
s = between_replace(s, start, end, replacement, end, 'stack judgement renderer')

wrap_start = "  function wrapStackJudgementEffects(){"
wrap_end = "  function clearRoofVibration(){"
wrap_replacement = r'''  function wrapStackJudgementEffects(){
    try{
      ...
      const wrapped=function(message){
        ...
        if(stack&&/^(?:PERFECT|GREAT!|GOOD!|아슬아슬!|MISS!)/.test(msg)){
          ensureStyle();
          if(showStackJudgementBurst(msg))return
        }
...
'''
s = between_replace(s, wrap_start, wrap_end, wrap_replacement, wrap_end, 'stack judgement wrapper')
p.write_text(s, encoding='utf-8')


# ------------------------------------------------------------------
# 2) KF-21 폭격 진동을 실제 폭발 원이 나타나는 프레임과 동기화
# ------------------------------------------------------------------
...
new_block = r'''    if(elapsed>610){
     ...
     if(kfDetonations>0){
      ...
      try{if(navigator&&typeof navigator.vibrate==='function'){navigator.vibrate(0);navigator.vibrate(kfDetonations>=3?78:kfDetonations===2?60:44)}else buzz(kfDetonations>=3?[55,10,78]:kfDetonations===2?60:44)}catch(_){try{buzz(kfDetonations>=3?[55,10,78]:kfDetonations===2?60:44)}catch(__){}}
     }
    }'''
if old_block not in h:
    raise SystemExit('missing KF21 explosion block')
